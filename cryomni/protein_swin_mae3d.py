...
from .model.torch_utils import get_3d_sincos_pos_embed, pad_tensor
...
class SwinTransformer_MAE3D_New(nn.Module):  # TODO: change to 3D
    ...
    def __init__(
        ...
        self.encoder_up_3 = UnetrUpBlock(
        ...
        self.encoder_up_2 = UnetrUpBlock(
        ...
        self.encoder_up_1 = UnetrUpBlock(
        ...
        self.encoder_up_0 = UnetrUpBlock(
        ...
        #                         nn.Conv3d(embed_dim // 4, 1, kernel_size=1)
        #                     )
        ...
        self.decoder_layers = nn.Sequential(
            ...
            nn.Linear(embed_dim // 8, embed_dim // 16),
            ...
            nn.Linear(embed_dim // 32, embed_dim // 64),
            nn.LayerNorm(embed_dim // 64),
            nn.SiLU(inplace=True),

            nn.Linear(embed_dim // 64, 1),
            # nn.Linear(embed_dim // 16, 1)
        )

        for m in self.decoder_layers.children():
            if isinstance(m, nn.Linear):
    ...
    def from_pretrained(cls, load_directory):
        ...
        model = cls(**{k: v for k, v in config.items() if k in valid_keys})
    ...
    def forward_encoder(self, x):
        ...
        dec3 = self.encoder_up_3(features[3], features[2])
        dec2 = self.encoder_up_2(dec3, features[1])
        dec1 = self.encoder_up_1(dec2, features[0])

        return dec1, mask_patches

    def forward_decoder(self,x):
        out = self.encoder_up_0(x)
    ...
    def forward_pred(self, x):
        """Return masked input, predicted masked density, and visible input.

        Call ``eval()`` and use ``torch.inference_mode()`` for inference.
        """
        x, mask = self.transform(x)
        x = torch.cat((x), dim=0)
        mask = torch.cat((mask), dim=0).to(x.device)
        embedings = self.patch_partition(x)
        embedings = embedings + self.pos_embed.type_as(x).to(x.device).clone().detach()
        latent, mask_patches = self.forward_encoder(embedings)
        pred = self.alpha_activation(self.forward_decoder(latent))
        mask_patches = F.interpolate(mask_patches.permute(0, 4, 1, 2, 3).float(), scale_factor=4, mode='nearest')
        return x * mask_patches, pred * mask_patches, x * (1 - mask_patches)
