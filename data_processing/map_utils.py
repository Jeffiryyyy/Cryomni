...
def segment_map(input_map,output_map,contour=0):
    ...
    with mrcfile.open(input_map, permissive=True) as mrc:
        ...
        input_data = mrc.data
        if not np.isfinite(contour) or not np.isfinite(input_data).all():
            raise ValueError("Map densities and contour threshold must be finite")
        useful_index = np.argwhere(input_data>contour)
        if useful_index.size == 0:
            raise ValueError(f"No density above contour threshold {contour} in {input_map}")
        ...
        new_data = input_data[min_x:max_x+1,min_y:max_y+1,min_z:max_z+1]
...
def write_voxel_map(actual_location,save_path, origin_map_path,segment_map_voxel):
...
def read_map_data(input_map_path):
