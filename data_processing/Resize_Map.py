    ...
    with torch.no_grad():
        ...
        with mrcfile.open(input_mrc, permissive=True) as orig_map:
            ...
            if orig_voxel_size[0]==1 and orig_voxel_size[1]==1 and orig_voxel_size[2]==1:
                if Path(input_mrc).absolute() == Path(output_mrc).absolute():
                    raise ValueError("Input and output map paths must differ")
                if os.path.lexists(output_mrc):
                    os.remove(output_mrc)
                os.symlink(str(Path(input_mrc).resolve()),output_mrc)
                #save time for 1*1*1 grid size map
                return
            data = orig_map.data.astype(np.float32)
            orig_data = torch.from_numpy(data.copy()).unsqueeze(0).unsqueeze(0)
